...
# There is already another RenderFarmNode in the API...
class _RenderFarmNode:
    ...
    def GetKey(self):
        return _RenderFarmNode.Key(self.address, self.port)
    ...
    def __str__(self):
        return (
            ...
            + str(self.port)
            + ", "
            + str(self.discoveryType)
            + ", "
            + str(self.state)
            + ", "
            + time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(self.lastContactTime)
            )
            + "]"
        )
...
class RenderFarm:
    ...
    def DiscoveredNode(self, address, port, discoveryType):
        with self.lock:
            ...
            if (key := _RenderFarmNode.Key(address, port)) in self.nodes:
                node = self.nodes[key]
                ...
                if node.state == NodeState.FREE:
                    ...
                    if self.currentJob:
                        logger.info("Retrying node: %s", key)
                ...
                # It is a new node
                logger.info("Discovered new node: %s", key)
                node = _RenderFarmNode(address, port, discoveryType)
                self.nodes[key] = node
