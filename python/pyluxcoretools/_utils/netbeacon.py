...
class NetBeaconSender:
    ...
    def Start(self):
        ...
        self.thread = threading.Thread(
            target=functools.partial(NetBeaconSender.__BeaconThread, self)
        )
    ...
    def __BeaconThread(self):
        ...
        while not self.stopEvent.is_set():
            logger.debug("NetBeaconSender LUXNETPING sent: %s", str(pingMsg))

            self.socket.sendto(
                pingMsg, (self.broadCastAddress, BROADCAST_PORT)
            )
...
class NetBeaconReceiver:
    ...
    def Start(self):
        ...
        # Create the thread
        self.thread = threading.Thread(
            target=functools.partial(NetBeaconReceiver.__BeaconThread, self)
        )
    ...
    def __BeaconThread(self):
        ...
        try:
            while not self.stopEvent.is_set():
                ...
                logger.debug(
                    "NetBeaconReceiver LUXNETPING received from %s: %s",
                    str(whereFrom),
                    str(data),
                )
