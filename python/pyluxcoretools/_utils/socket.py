...
def RecvLine(soc):
    if not (data := soc.recv(BUFF_SIZE)):
        raise RuntimeError("Unable to receive a line")

    sio = io.StringIO(data.decode("utf-8"))

    # I potentially return only the first line but this function should be use
    # to receive only one line anyway
...
def SendFile(soc, fileName):
    ...
    logger.info(
        "Transferred %s in %s (%s/sec)",
        DataSize(size),
        time.strftime("%H:%M:%S", time.gmtime(dt)),
        DataSize(0.0 if dt <= 0.0 else size / dt),
    )


def RecvFile(soc, fileName):
    logger.info("Receiving file: %s", fileName)
    ...
    with open(fileName, "wb") as f:
        ...
        while recvData < size:
            ...
            if len(data) > 0:
                ...
                raise RuntimeError(
                    "Interrupted transmission while receiving file: "
                    + fileName
                )
